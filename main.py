...
class HorriaLabApp(tk.Tk):
    ...
    def _build_layout(self):
        ...
        # ملاحظات
        ttk.Label(form, text="ملاحظات:").grid(row=2, column=1, sticky="e", padx=8, pady=6)
        self.notes_entry = ttk.Entry(form, width=25)
        self.notes_entry.grid(row=2, column=0, sticky="w", padx=8, pady=6)
        ...
        cols = ("التاريخ", "نوع الوارد", "اسم المورد", "اسم المنتج", "الحالة", "ملاحظات")
    ...
    def _get_suppliers(self):
        if not self.connected:
            return []
        return self.sm.get_unique_suppliers(incoming_type=self.type_cb.get() or None)

    def _get_products(self):
        if not self.connected:
            return []
        return self.sm.get_unique_products(
    ...
    def save_record(self):
        ...
        try:
            self.sm.append_record(date_str, incoming_type, supplier, product, status, notes)
        ...
        for r in self.records_cache:
            if str(r.get("التاريخ", "")).strip() == today_str:
                self.tree.insert("", "end", values=(
                    ...
                    r.get("اسم المنتج", ""), r.get("الحالة", ""), r.get("ملاحظات", ""),
                ))
