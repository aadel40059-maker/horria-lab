...
def daily_report(records, date_str, out_path):
    ...
    if day_rows:
        col_headers = ["ملاحظات", "الحالة", "اسم المنتج", "اسم المورد", "نوع الوارد"]
        rows = []
        for r in day_rows:
            rows.append([
                ar(r.get("ملاحظات", "") or "-"),
        ...
        widths = [4 * cm, 2.3 * cm, 3.5 * cm, 3.5 * cm, 4.2 * cm]
...
def monthly_rejected_report(records, year, month, out_path):
    ...
    if rejected:
        col_headers = ["ملاحظات", "اسم المنتج", "اسم المورد", "نوع الوارد", "التاريخ"]
        rows = []
        for r in rejected:
            rows.append([
                ar(r.get("ملاحظات", "") or "-"),
        ...
        widths = [4 * cm, 3.3 * cm, 3.3 * cm, 3.4 * cm, 2.5 * cm]
        flow.append(_build_table(rows, col_headers, widths))
...
def supplier_report(records, supplier_query, out_path):
    ...
    if supplier_rows:
        col_headers = ["ملاحظات", "الحالة", "اسم المنتج", "نوع الوارد", "التاريخ"]
        rows = []
        ...
        widths = [3.5 * cm, 2.3 * cm, 3.5 * cm, 3.5 * cm, 2.7 * cm]
        flow.append(_build_table(rows, col_headers, widths))
    else:
